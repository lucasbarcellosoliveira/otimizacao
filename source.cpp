...
typedef double type; //tipo a ser utilizado ao longo do calculo: relavante para precisão

using namespace std;

int iterBusca=0; //numero de iteracoes de bsuca (secao aurea ou almijo)
...
type almijo(valarray<type> x, valarray<type> d, type gama=0.75, type n=0.5){ //busca de Almijo
       type t=1;
       while (f(x+t*d)>f(x)+n*t*(df(x)*d).sum()){
             t*=n;
             iterBusca++;
       }
       return t;
...
valarray<type> gradiente(valarray<type> x, bool usarAlmijo=true, type tol=0.00001){ //metodo do gradiente
     ...
     while (pow(nabda[0],2)+pow(nabda[1],2)>pow(tol,2)&&(x0!=x)[0]&&(x0!=x)[1]&&iterMetodo<5000){ //condicao de parada: gradiente aprox igual a 0, duas iteracoes com mesmo otimo e numero de iteracoes maximo (5000)
           x0=x;
           if (usarAlmijo)
              x+=almijo(x,-nabda)*-nabda; //chamada a busca de Almijo
           else
